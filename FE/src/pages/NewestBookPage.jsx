      ...
      const getAllBooksbyType = async () => {
        try {
          ...
          const requests = categories.map((category) =>
            axios.get(`http://localhost:5000/api/book/books-newest/categories/${category.label}`)
          );

          const responses = await Promise.all(requests);

          const fullListNewest = responses.flatMap((res) => res.data.data);

          console.log(fullListNewest);
          setAllBooks(fullListNewest);
        } catch (err) {
          console.error(err);
        } finally {
          setIsLoading(false);
  ...
  const getListBooksByCategory = async (category) => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:5000/api/book/books-newest/categories/${category}`,
      );
